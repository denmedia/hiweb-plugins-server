<?php
	...
	if(get_option(HW_PLUGINS_SERVER_OPTIONS_STATUS) === false) add_option(HW_PLUGINS_SERVER_OPTIONS_STATUS,'0');
	...
	function _hw_ps_options(){
		...
		add_submenu_page(
			'options-general.php',
			'hiWeb Server Settings',
			'hiWeb Plugins Server',
			'activate_plugins',
			HW_PLUGINS_SERVER_OPTIONS_PAGE_SLUG,
			'_hw_plugins_server_options_settings'
		);

		add_submenu_page(
			'plugins.php',
			'hiWeb Remote Plugins',
			'hiWeb Remote Plugins',
			'activate_plugins',
			HW_PLUGINS_SERVER_REMOTE_PAGE_SLUG,
			'_hw_plugins_server_remote_page'
		);


		if(hw_plugins_server()->get_server_status()){
			add_menu_page(
				'Plugins Server',
				'Plugins Server',
				'activate_plugins',
				HW_PLUGINS_SERVER_PAGE_SLUG,
