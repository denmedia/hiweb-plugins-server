<?php
	...
	function _hw_plugins_server_plugin_hosted() {
		...
		$slug = $_POST['plugin'];
		if ( $do == 'hosted' ) {
			$R = hiweb_plugins_server()->do_server_plugin_hosted( $slug );
		}
		if ( $do == 'unhosted' ) {
			$R = hiweb_plugins_server()->do_server_plugin_hosted( $slug, false );
		}
		ob_start();
	...
	function _hw_plugins_server_get() {
		$R = array(
			'status'       => hiweb_plugins_server()->host()->get_status(),
