...
<div class="wrap">
    ...
    <?php
        ...
        $plugins = hw_plugins_server()->get_remote_plugins();
        if (is_array($plugins)) : ?>
            ...
            <?php
                foreach ($plugins as $slug => $plugin) :
                    $isActive = hw_plugins_server()->is_plugin_active($slug);
                    $isExists = file_exists(WP_PLUGIN_DIR . '/' . $slug);
                    $currentVersion = hw_plugins_server()->get_plugin_version($slug);
                    $isUpdate = ($isExists && ($currentVersion != $plugin['Version']));
                    $trClass = array();
                    $trClass[] = $isExists ? 'active' : 'inactive';
                    if ($isUpdate) $trClass[] = 'update';
            ...
            ?><h2>Unable to get the list of plugins</h2><h3>Reason: <?php echo hw_plugins_server()->get_remote_status(null, true); ?></h3><?php
        endif;
